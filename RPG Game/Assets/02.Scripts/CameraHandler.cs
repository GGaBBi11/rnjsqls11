...
public class CameraHandler : MonoBehaviour
{
    ...
    [SerializeField] private float maxDistance = 30; // 카메라와 타겟간 최대 거리
    [SerializeField] private float wheelSpeed = 500; //  마우스 휠 속도
    [SerializeField] private float xPointSpeed = 500; // 마우스 x이동속도
    [SerializeField] private float yPointSpeed = 500; // 마우스 y이동속도
    private float yMinLimit = 5;
    private float yMaxLimit = 80;
    private float x, y; // 마우스 위치 (회전)
    private float distance; // 카메라와 타겟 간의 거리
    private Transform tr;

    private void Awake()
    {
        tr = GetComponent<Transform>();
        distance = Vector3.Distance(tr.position, tr.position);
        x = tr.eulerAngles.y;
        y = -tr.eulerAngles.x;
    }
